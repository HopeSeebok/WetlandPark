...
import com.yunwei.library.data.ChineseCalendar;
import com.yunwei.library.utils.CalendarUtil;
...
public class GregorianLunarCalendarView extends LinearLayout implements NumberPickerView.OnValueChangeListener{
    ...
    private Calendar adjustCalendarByLimit(Calendar c, int yearStart, int yearStop, boolean isGregorian){
        ...
        if(isGregorian){
            ...
            if(yearGrego > yearStop){
                ...
                int daySway = CalendarUtil.getSumOfDayInMonthForGregorianByMonth(yearStop, MONTH_STOP_GREGORIAN);
            ...
            if(Math.abs(yearGrego - yearStart) < Math.abs(yearGrego - yearStop)){
                ...
                int daySway = CalendarUtil.getSumOfDayInMonthForLunarByMonthLunar(yearStop, MONTH_STOP_LUNAR_NORMAL);
    ...
    private void setDisplayData(boolean isGregorian){
        ...
        if(isGregorian){
            ...
            if(mDisplayYearsLunar == null){
                ...
                for(int i = 0; i < YEAR_SPAN; i++){
                    mDisplayYearsLunar[i] = CalendarUtil.getLunarNameOfYear(i + YEAR_START);
                }
            }
            if(mDisplayMonthsLunar == null){
                mDisplayMonthsLunar = new String[MONTH_SPAN_GREGORIAN];
                for(int i = 0; i < MONTH_SPAN_GREGORIAN; i++){
                    mDisplayMonthsLunar[i] = CalendarUtil.getLunarNameOfMonth(i + 1);
                }
            }
            if(mDisplayDaysLunar == null){
                mDisplayDaysLunar = new String[DAY_SPAN_LUNAR];
                for(int i = 0; i < DAY_SPAN_LUNAR; i++){
                    mDisplayDaysLunar[i] = CalendarUtil.getLunarNameOfDay(i + 1);
                }
            }
        }
    }
    ...
    private void initValuesForM(ChineseCalendar cc, boolean isGregorian, boolean anim){
        ...
        if(isGregorian){
            ...
            int monthLeap = CalendarUtil.getMonthLeapByYear(cc.get(ChineseCalendar.CHINESE_YEAR));
            if(monthLeap == 0){
                ...
                monthSway = CalendarUtil.convertMonthLunarToMonthSway(cc.get(ChineseCalendar.CHINESE_MONTH), monthLeap);
                newDisplayedVales = CalendarUtil.getLunarMonthsNamesWithLeap(monthLeap);
            }
        }
    ...
    private void initValuesForD(ChineseCalendar cc, boolean isGregorian, boolean anim){
        if(isGregorian){
            ...
            int dayStop = CalendarUtil.getSumOfDayInMonthForGregorianByMonth(cc.get(Calendar.YEAR), cc.get(Calendar.MONTH) + 1);
            ...
            int dayStop = CalendarUtil.getSumOfDayInMonthForLunarByMonthLunar(cc.get(ChineseCalendar.CHINESE_YEAR), cc.get(ChineseCalendar.CHINESE_MONTH));
    ...
    private void passiveUpdateMonthAndDay(int oldYearFix, int newYearFix, boolean isGregorian){
        ...
        if(isGregorian){
            ...
            int oldDayStop = CalendarUtil.getSumOfDayInMonth(oldYearFix, oldMonthSway, true);
            int newDayStop = CalendarUtil.getSumOfDayInMonth(newYearFix, newMonthSway, true);
            ...
            int newYearFixMonthLeap = CalendarUtil.getMonthLeapByYear(newYearFix);//1.计算当前year是否有闰月
            int oldYearFixMonthLeap = CalendarUtil.getMonthLeapByYear(oldYearFix);//2.计算之前year是否有闰月

            if(newYearFixMonthLeap == oldYearFixMonthLeap){
                //only update day picker
                newMonthSway = oldMonthSway;

                int oldMonthLunar = CalendarUtil.convertMonthSwayToMonthLunar(oldMonthSway, oldYearFixMonthLeap);
                int newMonthLunar = CalendarUtil.convertMonthSwayToMonthLunar(newMonthSway, newYearFixMonthLeap);
                int oldDayStop = CalendarUtil.getSumOfDayInMonthForLunarByMonthLunar(oldYearFix, oldMonthLunar);
                int newDayStop = CalendarUtil.getSumOfDayInMonthForLunarByMonthLunar(newYearFix, newMonthLunar);
                ...
                //月视图需要更新
                mCurrDisplayMonthsLunar = CalendarUtil.getLunarMonthsNamesWithLeap(newYearFixMonthLeap);

                //优化方案
                int oldMonthLunar = CalendarUtil.convertMonthSwayToMonthLunar(oldMonthSway, oldYearFixMonthLeap);
                int oldMonthLunarAbs = Math.abs(oldMonthLunar);
                newMonthSway = CalendarUtil.convertMonthLunarToMonthSway(oldMonthLunarAbs, newYearFixMonthLeap);
                ...
                //日视图需要更新
                int oldDayStop = CalendarUtil.getSumOfDayInMonth(oldYearFix, oldMonthSway, false);
                int newDayStop = CalendarUtil.getSumOfDayInMonth(newYearFix, newMonthSway, false);
                if(oldDayStop == newDayStop){
                    return;//不需要更新
                }else{
                    int newDaySway = (oldDaySway <= newDayStop) ? oldDaySway : newDayStop;
    ...
    private void passiveUpdateDay(int oldYear, int newYear, int oldMonth, int newMonth, boolean isGregorian){
        ...
        int oldDayStop = CalendarUtil.getSumOfDayInMonth(oldYear, oldMonth, isGregorian);
        int newDayStop = CalendarUtil.getSumOfDayInMonth(newYear, newMonth, isGregorian);
    ...
    public static class CalendarData{
        ...
        private void initChineseCalendar(){
            if(isGregorian){
                ...
                int y = pickedYear;
                int m = CalendarUtil.convertMonthSwayToMonthLunarByYear(pickedMonthSway, pickedYear);
                int d = pickedDay;
