...
public class HistoryTrackData {
    ...
    public class Points {
        ...
        /**
         * {@link com.yunwei.water.common #Constants.TRACK_RECORD_POINT_STATE}#
         */
        public String state;//自定义字段->点的状态(du yang)
    ...
    public List<LngLatEntity> getListPoints() {
        List<LngLatEntity> list = new ArrayList<>();
        ...
        while (it.hasNext()) {
            ...
            if (Math.abs(location.get(0) - 0.0) < 0.01 && Math.abs(location.get(1) - 0.0) < 0.01) {
                ...
                LngLatEntity latLng = new LngLatEntity(location.get(0), location.get(1));
                list.add(latLng);
            }
