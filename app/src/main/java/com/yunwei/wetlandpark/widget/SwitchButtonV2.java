...
import com.yunwei.wetlandpark.utils.ColorUtils;
...
public class SwitchButtonV2 extends CompoundButton {
	...
	public SwitchButtonV2(Context context, AttributeSet attrs, int defStyle) {
		super(context, attrs, defStyle);
		init(attrs);
	}

	public SwitchButtonV2(Context context, AttributeSet attrs) {
		super(context, attrs);
		init(attrs);
	}
	...
	private void init(AttributeSet attrs) {
		...
		TypedArray ta = attrs == null ? null : getContext().obtainStyledAttributes(attrs, R.styleable.SwitchButtonV2);
		if (ta != null) {
			thumbDrawable = ta.getDrawable(R.styleable.SwitchButtonV2_kswThumbDrawable);
			thumbColor = ta.getColorStateList(R.styleable.SwitchButtonV2_kswThumbColor);
			margin = ta.getDimension(R.styleable.SwitchButtonV2_kswThumbMargin, margin);
			marginLeft = ta.getDimension(R.styleable.SwitchButtonV2_kswThumbMarginLeft, margin);
			marginRight = ta.getDimension(R.styleable.SwitchButtonV2_kswThumbMarginRight, margin);
			marginTop = ta.getDimension(R.styleable.SwitchButtonV2_kswThumbMarginTop, margin);
			marginBottom = ta.getDimension(R.styleable.SwitchButtonV2_kswThumbMarginBottom, margin);
			thumbWidth = ta.getDimension(R.styleable.SwitchButtonV2_kswThumbWidth, thumbWidth);
			thumbHeight = ta.getDimension(R.styleable.SwitchButtonV2_kswThumbHeight, thumbHeight);
			thumbRadius = ta.getDimension(R.styleable.SwitchButtonV2_kswThumbRadius, Math.min(thumbWidth, thumbHeight) / 2.f);
			backRadius = ta.getDimension(R.styleable.SwitchButtonV2_kswBackRadius, thumbRadius + density * 2f);
			backDrawable = ta.getDrawable(R.styleable.SwitchButtonV2_kswBackDrawable);
			backColor = ta.getColorStateList(R.styleable.SwitchButtonV2_kswBackColor);
			backMeasureRatio = ta.getFloat(R.styleable.SwitchButtonV2_kswBackMeasureRatio, backMeasureRatio);
			animationDuration = ta.getInteger(R.styleable.SwitchButtonV2_kswAnimationDuration, animationDuration);
			fadeBack = ta.getBoolean(R.styleable.SwitchButtonV2_kswFadeBack, true);
			tintColor = ta.getColor(R.styleable.SwitchButtonV2_kswTintColor, tintColor);
			textOn = ta.getString(R.styleable.SwitchButtonV2_kswTextOn);
			textOff = ta.getString(R.styleable.SwitchButtonV2_kswTextOff);
			textMarginH = Math.max(textMarginH, backRadius / 2);
			textMarginH = ta.getDimension(R.styleable.SwitchButtonV2_kswTextMarginH, textMarginH);
			autoAdjustTextPosition = ta.getBoolean(R.styleable.SwitchButtonV2_kswAutoAdjustTextPosition, autoAdjustTextPosition);
			ta.recycle();
		}
