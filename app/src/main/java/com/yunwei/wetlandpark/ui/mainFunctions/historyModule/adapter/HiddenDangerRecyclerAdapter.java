 ...
 */
public class HiddenDangerRecyclerAdapter  extends BaseRecyclerViewAdapter<TroubleTable>{

    private Context mContext;
    private TroubleTable mTable;

    public HiddenDangerRecyclerAdapter(Context context) {
        mContext = context;
    }
    ...
    public void onBindBaseViewHolder(RecyclerView.ViewHolder holder, final int position) {
        ...
        holders.itemWorkHistoryLayoutFirstTitleTextView.setText("紧急程度:" + Constants.TROUBLE_EMERGENCY_LEVEL.get(mTable.getEmergencyLevel()));
        holders.itemWorkHistoryLayoutSecondTitleTextView.setVisibility(View.GONE);
        holders.itemWorkHistoryLayoutThirdTitleTextView.setText("上报时间:" + IDateTimeUtils.formatDate(mTable.getTime()));

        //根据设施状态展示图标的样子
        if (((TroubleTable) mTable).getLocalTag() == Constants.LocalState.UNSUBMITTED) {
