...
public class ILog {
    ...
    private static void writeLogtoFile(String mylogtype, String tag, String text) {// 新建或打开日志文件
        String needWriteFiel = IDateTimeUtils.formatDate(System.currentTimeMillis(), "yyyy-MM-dd");
        String needWriteMessage = IDateTimeUtils.formatDate(System.currentTimeMillis(), "yyyy-MM-dd HH:mm:ss") + "    " + mylogtype
                + "    " + tag + "    " + text;
        File file = new File(IFileUtils.getLogDir() + needWriteFiel, needWriteFiel + "-Log.txt");

        if (!new File(IFileUtils.getLogDir() + needWriteFiel).exists()) {
            new File(IFileUtils.getLogDir() + needWriteFiel).mkdirs();
        }
