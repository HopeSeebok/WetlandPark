...
import com.yunwei.map.CompassView;
...
public class ArcGisBaseMapView extends MapView implements OnStatusChangedListener {
    ...
    protected ArcGISLocalTiledLayer localTiledLayer;

    private static DaoSession daoSession;

    protected double defaultScale = GoogleMapLayer.scales[14];
    ...
    protected ArcGisMapListener listener;
    protected boolean isExternalMapLayer = true;
...
//        setAllowRotationByPinch(true);
    ...
    private void initView() {
        ...
        zoominLayout.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View v) {
                MLog.d(TAG, "scale==" + getScale() + ", maxScale==" + getMaxScale() + ",minScale==" + getMinScale());
                MLog.d(TAG, "is >=" + (getScale() >= getMaxScale()));
//                if (getScale() >= getMaxScale()) {
//                    zoominLayout.setClickable(false);
//                    zoominLayout.setBackgroundColor(Color.parseColor("#ffffff"));
//                    zoominIv.setImageResource(R.mipmap.ic_track_explore_zoomin_dis);
//                } else {
                zoomin();
                MLog.d(TAG, "scale==" + getScale());
                zoominLayout.setClickable(true);
                zoomoutLayout.setClickable(true);
//                    zoominLayout.setBackgroundResource(R.drawable.zoomin_selector);
//                    zoomoutLayout.setBackgroundResource(R.drawable.zoomin_selector);
//                    zoominIv.setImageResource(R.mipmap.ic_track_explore_zoomin);
//                    zoomoutIv.setImageResource(R.mipmap.ic_track_explore_zoomout);
//                }
        ...
        zoomoutLayout.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View v) {
                MLog.i(TAG, "scale==" + getScale() + ", maxScale==" + getMaxScale() + ",minScale==" + getMinScale());
//                if (getScale() <= getMinScale()) {
//                    zoomoutLayout.setClickable(false);
//                    zoomoutLayout.setBackgroundColor(Color.parseColor("#ffffff"));
//                    zoomoutIv.setImageResource(R.mipmap.ic_track_explore_zoo  mout_dis);
//                } else {
                zoomout();
                zoominLayout.setClickable(true);
                zoomoutLayout.setClickable(true);
...
//                }
    ...
    public void onStatusChanged(Object o, STATUS status) {
        if (status == STATUS.LAYER_LOADED) {
            if (isExternalMapLayer) {
                double x = 1.2697781415967792E7;
                double y = 2579888.187931014;
                try {
                    x = Double.parseDouble(MSpfUtil.getValue(getContext(), Constant.LNG_FLAG, 1.2697781415967792E7).toString());
                    y = Double.parseDouble(MSpfUtil.getValue(getContext(), Constant.LAT_FLAG, 2579888.187931014).toString());
                } catch (Exception e) {
                ...
                setExtent(new Point(176067.955, 132194.812));
                setScale(800000);
            }
        }
    }
    ...
    public interface ArcGisMapListener {
        void onClickInsideBtton();
    }
}
